//
//  NSString+XYFileDownloadsExtend.m
//  XYFileDownloader
//
...
@implementation NSString (XYFile)
...
+ (NSString *)getDownloadLocalFolderPath {
    ...
    NSString *localFolderPath = [cacheFolder stringByAppendingPathComponent:@"XYFileDownloaderCacheFolder"];
...
+ (NSString *)getDownloadDisplayFolderPath {
    NSString *cacheFolder = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES).firstObject;
    NSString *localFolderPath = [cacheFolder stringByAppendingPathComponent:@"XYFileDownloaderDisplayFolder"];
    BOOL isDirectory, isExist;
    isExist = [[NSFileManager defaultManager] fileExistsAtPath:localFolderPath isDirectory:&isDirectory];
    if (!isExist || !isDirectory) {
        [[NSFileManager defaultManager] createDirectoryAtPath:localFolderPath withIntermediateDirectories:YES attributes:nil error:nil];
...
+ (NSString *)getICloudCacheFolder {
    NSString *cacheFolder = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES).firstObject;
    NSString *localFolderPath = [cacheFolder stringByAppendingPathComponent:@"XYFilesICloudDrive"];
    BOOL isDirectory, isExist;
    isExist = [[NSFileManager defaultManager] fileExistsAtPath:localFolderPath isDirectory:&isDirectory];
    if (!isExist || !isDirectory) {
        [[NSFileManager defaultManager] createDirectoryAtPath:localFolderPath withIntermediateDirectories:YES attributes:nil error:nil];
...
- (XYFileType)xy_fileType {
    ...
    {
        return XYFileTypeWindows;
    }
    return XYFileTypeOther;
}
...
+ (NSString *)getXYFileBrowserCachePath {
    ...
    NSString *fullPath = [cachePath stringByAppendingPathComponent:@"XYFileBrowserCache"];
...
+ (NSString *)getMarkupCachePath {
    NSString *cache = [self getXYFileBrowserCachePath];
