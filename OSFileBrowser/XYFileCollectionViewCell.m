...
@interface XYFileCollectionViewCell ()
...
@implementation XYFileCollectionViewCell {
    /////////// constraints ///////////
...
- (void)setStatus:(XYFileAttributeItemStatus)status {
    
    self.optionBtn.userInteractionEnabled = NO;
    self.contentView.layer.borderColor = [UIColor colorWithWhite:0.75 alpha:1.0].CGColor;
    
    switch (status) {
        case XYFileAttributeItemStatusDefault: {
            [self.optionBtn setImage:[UIImage XYFileBrowserImageNamed:@"grid-options"] forState:UIControlStateNormal];
            self.optionBtn.userInteractionEnabled = YES;
            break;
        }
        case XYFileAttributeItemStatusEdit: {
            [self.optionBtn setImage:[UIImage XYFileBrowserImageNamed:@"grid-selection"] forState:UIControlStateNormal];
            break;
        }
        case XYFileAttributeItemStatusChecked: {
            [self.optionBtn setImage:[UIImage XYFileBrowserImageNamed:@"grid-selected"] forState:UIControlStateNormal];
...
- (void)setFileModel:(XYFileAttributeItem *)fileModel {
    ...
    if (fileModel.isDirectory) {
        ...
        if (fileModel.isImage) {
            ...
            [self.iconView xy_imageWithMediaURL:videoURL placeholderImage:[UIImage XYFileBrowserImageNamed:@"table-fileicon-images"] completionHandlder:^(UIImage *image) {
...
- (void)renameFile {
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"好的" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (!moveError) {
            XYFileAttributeItem *oldItem = [self.fileModel mutableCopy];
...
- (UIImageView *)starImageView {
    if (!_starImageView) {
        ...
        imageView.image = [UIImage XYFileBrowserImageNamed:@"favorites-badge"];
