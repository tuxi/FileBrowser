...
@implementation XYFileHelper

+ (NSArray<XYFile *> *)sortByPinYinWithArray:(NSArray<XYFile *> *)toSortArray {
    //将传入数组转换为可变数组
    NSMutableArray<XYFile *> *needSortArray = [NSMutableArray arrayWithArray:toSortArray];
    //存储对应字母开头的所有数据的数组
    ...
    for(int i='A';i<='Z';i++){
        ...
        for(int j = 0; j < needSortArray.count; j++){
            XYFile *file = [needSortArray objectAtIndex:j];
...
NS_INLINE NSInteger displayNameSort(XYFile *file1, XYFile *file2, void *context) {
...
+ (NSArray<XYFile *> *)sortByCreateDateWithArray:(NSArray<XYFile *> *)toSortArray {
