...
NSString * const XYFileCollectionHeaderViewDefaultIdentifier = @"UICollectionReusableView";

@interface XYFileCollectionHeaderView ()
...
@implementation XYFileCollectionHeaderView

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [UIColor colorWithRed:31/255.0 green:31/255.0 blue:31/255.0 alpha:1.0];
        [self setupViews];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(sortTypeChangeNotification:) name:XYFileBrowserAppearanceConfigsSortTypeDidChangeNotification object:nil];
    }
    return self;
}
...
- (UIButton *)searchButton {
    if (!_searchButton) {
        ...
        dispatch_once(&onceToken, ^{
            searchImage = [[UIImage XYFileBrowserImageNamed:@"sortbar_search"] xy_changeImageColorWithColor:[UIColor whiteColor]];
        });
...
- (UIButton *)changeStyleButton {
    if (!_changeStyleButton) {
        ...
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
           normalImage = [[UIImage XYFileBrowserImageNamed:@"sortbar_grid_s"] xy_changeImageColorWithColor:[UIColor whiteColor]];
            selectedImage = [[UIImage XYFileBrowserImageNamed:@"sortbar_listview_s"] xy_changeImageColorWithColor:[UIColor whiteColor]];
        });
...
- (UISegmentedControl *)sortControl {
    if (!_sortControl) {
        ...
        _sortControl.selectedSegmentIndex = [XYFileBrowserAppearanceConfigs fileSortType];
    }
    return _sortControl;
}
...
//    if (_sortControl.selectedSegmentIndex != [XYFileBrowserAppearanceConfigs fileSortType]) {
        _sortControl.selectedSegmentIndex = [XYFileBrowserAppearanceConfigs fileSortType];
...
- (void)changeStyleButtonClick:(UIButton *)sender {
    ...
    XYFileCollectionLayoutStyle style = ![XYFileCollectionViewFlowLayout collectionLayoutStyle];
    [XYFileCollectionViewFlowLayout setCollectionLayoutStyle:style];
...
- (void)selectedSortChanged:(UISegmentedControl *)sortControl {
    [XYFileBrowserAppearanceConfigs setFileSortType:sortControl.selectedSegmentIndex];
