...
typedef enum {
    XYFileFlagsArchived         = 0x01,
    XYFileFlagsHidden           = 0x02,
    XYFileFlagsNoDump           = 0x04,
    XYFileFlagsOpaque           = 0x08,
    XYFileFlagsSystemAppendOnly = 0x10,
    XYFileFlagsSystemImmutable  = 0x20,
    XYFileFlagsUserAppendOnly   = 0x40,
    XYFileFlagsUserImmutable    = 0x80
}
XYFileFlags;

@interface XYFile : NSObject <NSCopying, NSMutableCopying> {
    @protected
    NSString *_path;
}
...
@property (atomic, readonly) XYFileFlags   flags;
...
@property (atomic, readonly) XYFile      * targetFile;
...
/// 初始化方法，根据文件路径创建一个XYFile对象
/// @param filePath 文件完整路径
/// @param hideDisplayFiles 是否显示隐藏文件，默认显示的,
/// @param error 返回错误信息
/// @return XYFile对象，当文件不存在获取读取文件失败时，return nil
