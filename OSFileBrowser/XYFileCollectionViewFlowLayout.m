//
//  XYFileCollectionViewFlowLayout.m
//  FileBrowser
//
...
NSNotificationName const XYFileCollectionLayoutStyleDidChangeNotification = @"XYFileCollectionLayoutStyleDidChangeNotification";

static OSLineDimensionType const OSLineDimensionTypeDefault = OSLineDimensionTypeSize;
static NSString * const XYFileCollectionViewFlowLayoutNumItemsOnLineKey = @"XYFileCollectionViewFlowLayoutNumItemsOnLineKey";
...
@interface XYFileCollectionViewFlowLayout ()
...
@implementation XYFileCollectionViewFlowLayout
...
+ (XYFileCollectionLayoutStyle)collectionLayoutStyle {
    NSNumber *res = [[NSUserDefaults standardUserDefaults] objectForKey:XYFileCollectionViewFlowLayoutNumItemsOnLineKey];
    if (!res) {
        return XYFileCollectionLayoutStyleMultipleItemOnLine;
    }
    return (XYFileCollectionLayoutStyle)[res integerValue];
}

+ (void)setCollectionLayoutStyle:(XYFileCollectionLayoutStyle)collectionLayoutStyle {
    if ([self collectionLayoutStyle] == collectionLayoutStyle) {
        return;
    }
    [[NSUserDefaults standardUserDefaults] setObject:@(collectionLayoutStyle) forKey:XYFileCollectionViewFlowLayoutNumItemsOnLineKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
    [[NSNotificationCenter defaultCenter] postNotificationName:XYFileCollectionLayoutStyleDidChangeNotification object:nil userInfo:@{@"XYFileCollectionLayoutStyle": @(collectionLayoutStyle)}];
}

@end

@interface XYFileCollectionViewFlowLayout (InspectableScrolling)

@property (nonatomic, assign) IBInspectable BOOL verticalScrolling;

@end

@implementation XYFileCollectionViewFlowLayout (InspectableScrolling)
