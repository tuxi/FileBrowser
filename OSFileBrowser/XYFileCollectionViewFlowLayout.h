//
//  XYFileCollectionViewFlowLayout.h
//  FileBrowser
//
...
FOUNDATION_EXPORT NSNotificationName const XYFileCollectionLayoutStyleDidChangeNotification;

typedef NS_ENUM(NSInteger, XYFileCollectionLayoutStyle) {
    XYFileCollectionLayoutStyleMultipleItemOnLine, // 一行可以有多个item
    XYFileCollectionLayoutStyleSingleItemOnLine, // 一行只有一个item
};

@interface XYFileCollectionViewFlowLayout : UICollectionViewFlowLayout
...
/// 是否从新的一行开始 决定每一组其实的坐标是否与上一组相连接
@property (nonatomic, assign) IBInspectable BOOL sectionsStartOnNewLine;
/// 行的宽高，默认设置此属性后，宽和高度相同, 如果不设置会自动根据各间距自动计算
...
@property (nonatomic, assign, class) XYFileCollectionLayoutStyle collectionLayoutStyle;
