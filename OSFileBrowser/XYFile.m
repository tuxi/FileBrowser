...
static NSNotificationName XYFileDidMarkupedNotification = @"XYFileDidMarkupedNotification";
static NSNotificationName XYFileDidCancelMarkupedNotification = @"XYFileDidCancelMarkupedNotification";
...
@property (atomic) XYFileFlags   flags;
...
@property (atomic) XYFile      * targetFile;
...
- (void)registerMotification {
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateMarkupState:) name:XYFileDidMarkupedNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateMarkupState:) name:XYFileDidCancelMarkupedNotification object:nil];
}
...
- (BOOL)reloadFileWithError:(NSError *__autoreleasing *)error {
    ...
    if(_isSymbolicLink == YES) {
        ...
        _targetFile = [[XYFile alloc] initWithPath:symLinkTarget];
    }
...
- (void)getFlags
{
    ...
    if(fileStat.st_flags & SF_ARCHIVED)   { _flags |= XYFileFlagsArchived;            }
    if(fileStat.st_flags & UF_HIDDEN)     { _flags |= XYFileFlagsHidden;              }
    if(fileStat.st_flags & UF_NODUMP)     { _flags |= XYFileFlagsNoDump;              }
    if(fileStat.st_flags & UF_OPAQUE)     { _flags |= XYFileFlagsOpaque;              }
    if(fileStat.st_flags & SF_APPEND)     { _flags |= XYFileFlagsSystemAppendOnly;    }
    if(fileStat.st_flags & SF_IMMUTABLE)  { _flags |= XYFileFlagsSystemImmutable;     }
    if(fileStat.st_flags & UF_APPEND)     { _flags |= XYFileFlagsUserAppendOnly;      }
    if(fileStat.st_flags & UF_IMMUTABLE)  { _flags |= XYFileFlagsUserImmutable;       }
}
...
- (void)getIcon {
    ...
    if(infos.isDirectory == YES) {
        baseIcon = [UIImage XYFileBrowserImageNamed:@"table-folder.png"];
    }
    else if (infos.isArchive) {
        baseIcon = [UIImage XYFileBrowserImageNamed:@"table-fileicon-archive"];
    }
    else if (infos.isWindows) {
        baseIcon = [UIImage XYFileBrowserImageNamed:@"table-foder-windows-smb"];
    }
    else {
        baseIcon = [UIImage XYFileBrowserImageNamed:@"table-fileicon-c-source"];
    }
...
- (BOOL)markup {
    ...
    if (res) {
        [[NSNotificationCenter defaultCenter] postNotificationName:XYFileDidMarkupedNotification object:self.path];
    }
    return res;
}
...
- (BOOL)cancelMarkup {
    ...
    if (res) {
        [[NSNotificationCenter defaultCenter] postNotificationName:XYFileDidCancelMarkupedNotification object:self.path];
    }
    return res;
}

/// 添加一个文件进行标记
+ (BOOL)addMarkupWithFile:(XYFile *)file {
    if (!file.path) {
        return NO;
    }
...
/// 添加一个文件进行标记
+ (BOOL)removeMarkupWithFile:(XYFile *)file {
    if (!file.path) {
        return NO;
    }
...
+ (NSArray<XYFile *> *)markupFilesWithNeedReload:(BOOL)reload {
    ...
    [array enumerateObjectsUsingBlock:^(NSString *  _Nonnull path, NSUInteger idx, BOOL * _Nonnull stop) {
        XYFile *file = [XYFile fileWithPath:path error:nil];
        if (file) {
...
- (void)updateMarkupState:(NSNotification *)note {
    ...
    if ([note.name isEqualToString:XYFileDidMarkupedNotification]) {
        _alreadyMarked = YES;
    }
    else if ([note.name isEqualToString:XYFileDidCancelMarkupedNotification]) {
        _alreadyMarked = NO;
    }
}
